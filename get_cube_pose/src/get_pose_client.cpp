...
private:
  ...
  void result_callback(const GoalHandleFind::WrappedResult &result) {
    ...
    RCLCPP_INFO(
          this->get_logger(), "X: %f",
          result.result->support_surfaces[0].primitive_poses[0].position.x);
      RCLCPP_INFO(
          this->get_logger(), "Y: %f",
          result.result->support_surfaces[0].primitive_poses[0].position.y);
      RCLCPP_INFO(
          this->get_logger(), "Z: %f",
          result.result->support_surfaces[0].primitive_poses[0].position.z);
  }
};
